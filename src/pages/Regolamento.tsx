...
const Regolamento = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const {
        data: chaptersData
      } = await supabase.from('regolamento_chapters').select('*').order('order_index');
      const {
        data: sectionsData
      } = await supabase.from('regolamento_sections').select('*').order('order_index');
      if (chaptersData) {
        ...
        const allDates = [...chaptersData.map(c => c.updated_at), ...(sectionsData?.map(s => s.updated_at) || [])];
        ...
        <main className="pt-32 pb-16 px-4">
          <div className="max-w-6xl mx-auto">
            <div className="mb-8">
              ...
              {lastUpdated && <p className="text-sm text-muted-foreground mt-2">
            ...
            {loading ? <div className="flex justify-center py-12">
              ...
              </div> : chapters.length === 0 ? <div className="bg-card/50 backdrop-blur-sm border border-primary/20 rounded-2xl p-8">
                ...
                <div className="lg:w-72 shrink-0">
                  <nav className="lg:sticky lg:top-32 space-y-1">
                    {chapters.map((chapter, index) => <button key={chapter.id} onClick={() => setActiveChapter(chapter.id)} className={cn("w-full text-left px-4 py-3 rounded-lg transition-all text-sm", activeChapter === chapter.id ? "bg-primary text-primary-foreground font-medium" : "text-muted-foreground hover:text-foreground hover:bg-muted")}>
                ...
                <div className="flex-1 min-w-0">
                  {activeChapterData && <div className="bg-card/50 backdrop-blur-sm border border-primary/20 rounded-2xl p-6 md:p-8">
                      <h2 className="text-2xl md:text-3xl font-russo mb-8 text-foreground">
                        {activeChapterIndex + 1}. {activeChapterData.title}
                      </h2>

                      <div className="space-y-8">
                        {activeChapterData.sections.map((section, sectionIndex) => <div key={section.id} className="border-l-2 border-primary/30 pl-6">
                        ...
                        {activeChapterData.sections.length === 0 && <p className="text-muted-foreground italic">
